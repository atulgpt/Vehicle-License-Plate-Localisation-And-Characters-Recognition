...
AREA_THRES = [2000, 15000];
AREA_CHAR_THRES = [500, 10000]; %min and max threshold fixed values
...
for i =1:sample_size-2
    ...
    for j = 1 : numel(stats1)
        ...
        if (stats1(j).Extent) > 0.80 && (stats1(j).Area > AREA_THRES(1)) && (stats1(j).Area < AREA_THRES(2))
            temp = temp + (L == j);
    ...
    if selected_regions == 1
        ...
        temp = ones(license_coordinates(4)+4,license_coordinates(3)+4);
        img_final = img_grey(license_coordinates(2)-2:license_coordinates(2) + license_coordinates(4)+1,license_coordinates(1)-2:license_coordinates(1) + license_coordinates(3)+1).*logical(temp);
    else
...
% find both black and white regions
    ...
    for j = 1:numel(stats2)
        ...
        tempFile = '';
        if count < 3
         for k = 13:size(template_list,2)
            temp_char = eval(template_list{k});
            %temp_char = imresize(temp_char,size(img_char));
            corr = corr2(img_char, temp_char);
            template_list{k}(end);
            if template_list{k}(end) == 'I' || template_list{k}(end) == '4'
                %figure, imshow(temp_char);
                %title('Template character I');
                corr;
            end
            corr;
            if corr > tempCorr
